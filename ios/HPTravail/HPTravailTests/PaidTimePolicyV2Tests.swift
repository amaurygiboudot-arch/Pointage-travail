...
final class PaidTimePolicyV2Tests: XCTestCase {
    ...
    func testUnknownPauseStatusDoesNotInventPaidOrUnpaidClassification() {
        let result = PaidTimePolicyV2.assess(
            sessionStart: at(8),
            sessionEnd: at(16),
    ...
    func testOpenPaidPauseUsesObservationTimeWithoutChangingItsStatus() {
        let result = PaidTimePolicyV2.assess(
            sessionStart: at(8),
            sessionEnd: nil,
            pauses: [PaidPauseFactV2(start: at(10), end: nil, paid: true)],
            until: at(10, 30)
        )

        XCTAssertEqual(result.paidPauseDuration, 30 * minute, accuracy: 0.001)
        XCTAssertEqual(result.paidDuration, 2 * 3_600 + 30 * minute, accuracy: 0.001)
        XCTAssertTrue(result.reliable)
    }
    ...
    func testPauseOutsideRequestedSessionDoesNotDegradeReliability() {
        let result = PaidTimePolicyV2.assess(
            sessionStart: at(8),
            sessionEnd: at(16),
            pauses: [PaidPauseFactV2(start: at(17), end: at(18), paid: nil)],
            until: at(16)
        )
