...
class V2RightsRestView @JvmOverloads constructor(
    ...
    init {
        ...
        addView(Button(context).apply {
            text = "➕ AJOUTER / METTRE À JOUR UN COMPTEUR"; isAllCaps = false; textSize = 14f
            setBackgroundResource(R.drawable.hp_panel); setOnClickListener { showCounterDialog() }
        }, LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, dp(52)))
        addView(content.apply { textSize = 13f; setPadding(0, dp(8), 0, 0) })

        addView(TextView(context).apply { text = "ABSENCES"; textSize = 15f; setPadding(0, dp(18), 0, dp(4)) })
        addView(TextView(context).apply {
            text = "Absence non rémunérée, arrêt maladie, congé payé ou autre : HoraTrack enregistre le cas réel. Pour un arrêt maladie, IJSS, maintien, subrogation et éventuelle prévoyance qui chevauche le maintien restent séparés pour éviter tout double compte."
            textSize = 12f; setPadding(0, 0, 0, dp(6))
        })
        addView(Button(context).apply {
            text = "➕ ABSENCE NON RÉMUNÉRÉE"; isAllCaps = false; textSize = 14f
        ...
        addView(Button(context).apply {
            text = "➕ AUTRE ABSENCE"; isAllCaps = false; textSize = 14f
            setBackgroundResource(R.drawable.hp_panel); setOnClickListener { chooseAbsenceType() }
    ...
    fun refresh() {
        val balances = if (companyId.isBlank()) V2RightsStore.all(context) else V2RightsStore.forCompany(context, companyId)
        val rights = if (balances.isEmpty()) "Compteurs : aucun renseigné pour cette entreprise." else balances.joinToString("\n") { b ->
            buildString {
        ...
        val sessions = if (companyId.isBlank()) V2RuntimeStore.allSessions(context) else {
        ...
        val rest = latest?.let { "Dernier repos entre journées : ${duration(it.restMs)} • conformité légale : À confirmer selon la règle applicable" }
            ?: "Repos quotidien : pas encore assez de journées terminées pour calculer un intervalle."
        val warnings = if (companyId.isBlank()) V2RightsStore.snapshot(context).warnings else V2RightsStore.snapshot(context, companyId = companyId).warnings
    ...
    private fun refreshAbsences() {
        ...
        val absences = if (companyId.isBlank()) emptyList() else V2RightsStore.absencesForCompany(context, companyId).sortedByDescending { it.startMs }
        if (absences.isEmpty()) {
            absenceBox.addView(TextView(context).apply { text = "Aucune absence enregistrée pour cette entreprise."; textSize = 13f; setPadding(0, dp(8), 0, 0) })
            return
        }
        absences.forEach { absence ->
            ...
            val sickness = if (absence.type == AbsencePayrollImpactV2.TYPE_SICKNESS) V2PayslipStore.sicknessAllowanceForAbsence(context, companyId, absence) else null
            val sicknessFlow = if (absence.type == AbsencePayrollImpactV2.TYPE_SICKNESS) SicknessPaymentFlowV2.resolve(absence, sickness) else null
            val sicknessAmount = if (absence.type == AbsencePayrollImpactV2.TYPE_SICKNESS) V2PayslipStore.sicknessTheoreticalNetForAbsence(context, companyId, absence) else null
            val relay = if (absence.type == AbsencePayrollImpactV2.TYPE_SICKNESS) V2PayslipStore.sicknessProvidentRelayForAbsence(context, companyId, absence) else null
            val row = LinearLayout(context).apply { orientation = HORIZONTAL; setPadding(0, dp(6), 0, 0) }
            row.addView(TextView(context).apply {
                text = buildString {
                    append("• ").append(AbsencePayrollImpactV2.label(absence.type)).append('\n')
                    append("  ").append(start.format(dateFormat)).append(" → ").append(end.format(dateFormat)).append(" • ").append(days).append(" jour").append(if (days > 1) "s" else "")
                    ...
                    if (sickness != null) {
                        ...
                        if (sickness.complete && sickness.dailyGross != null && sickness.payableDays != null && sickness.estimatedGrossTotal != null) {
                            append(String.format(Locale.FRANCE, "%.2f € brut/jour • %d jour(s) indemnisable(s) • %.2f € brut estimés", sickness.dailyGross, sickness.payableDays, sickness.estimatedGrossTotal)).append(" (si droits ouverts)")
                        } else {
                            append("à confirmer"); sickness.warnings.firstOrNull()?.let { append(" — ").append(it) }
                        }
                    }
                    if (sicknessFlow != null) {
                        ...
                        when (sicknessFlow.ijssRecipient) {
                            SicknessPaymentFlowV2.IjssRecipient.EMPLOYER -> {
                                ...
                                sicknessFlow.employerIjssReimbursementGross?.let { append(String.format(Locale.FRANCE, " • %.2f € brut estimés", it)) }
                            ...
                            SicknessPaymentFlowV2.IjssRecipient.EMPLOYEE -> {
                                ...
                                sicknessFlow.directEmployeeIjssGross?.let { append(String.format(Locale.FRANCE, " • %.2f € brut estimés", it)) }
                            }
                    ...
                    sicknessAmount?.employerComplementBeforeProvidentNet?.let {
                        append('\n').append("  Complément avant prévoyance : ").append(String.format(Locale.FRANCE, "%.2f € net avant PAS", it))
                    }
                    if (sicknessAmount?.finalComplementReliable == true) {
                        sicknessAmount.employerProvidentNetDeducted?.let {
                            append('\n').append("  Prévoyance chevauchante déduite : ").append(String.format(Locale.FRANCE, "%.2f € net", it))
                        }
                        sicknessAmount.employerComplementFinalNet?.let {
                            append('\n').append("  Complément employeur final estimé : ").append(String.format(Locale.FRANCE, "%.2f € net avant PAS", it))
                        }
            ...
            row.addView(Button(context).apply {
                text = "SUPPRIMER"; isAllCaps = false; textSize = 11f; setBackgroundResource(R.drawable.hp_panel)
    ...
    private fun chooseAbsenceType() {
        if (companyId.isBlank()) { Toast.makeText(context, "Choisis d’abord une entreprise", Toast.LENGTH_LONG).show(); return }
        val labels = arrayOf("Arrêt maladie", "Congé payé", "Accident du travail", "Maternité / paternité", "Autre absence")
        val types = arrayOf(AbsencePayrollImpactV2.TYPE_SICKNESS, AbsencePayrollImpactV2.TYPE_PAID_LEAVE, AbsencePayrollImpactV2.TYPE_WORK_ACCIDENT, AbsencePayrollImpactV2.TYPE_PARENTAL, AbsencePayrollImpactV2.TYPE_OTHER)
        AlertDialog.Builder(context).setTitle("Type d’absence").setItems(labels) { _, which -> showAbsencePeriodDialog(types[which], AbsenceSalaryTreatmentV2.TO_CONFIRM, false) }.setNegativeButton("ANNULER", null).show()
    }

    private fun showAbsencePeriodDialog(type: String, initialTreatment: AbsenceSalaryTreatmentV2, treatmentLocked: Boolean) {
        if (companyId.isBlank()) { Toast.makeText(context, "Choisis d’abord une entreprise", Toast.LENGTH_LONG).show(); return }
        ...
        startButton.setOnClickListener { pick(start) { picked -> start = picked; if (end.isBefore(start)) end = start; updateLabels() } }
        ...
        if (!treatmentLocked) {
            treatmentButton.setOnClickListener {
                ...
                val values = arrayOf(AbsenceSalaryTreatmentV2.FULLY_MAINTAINED, AbsenceSalaryTreatmentV2.PARTIALLY_MAINTAINED, AbsenceSalaryTreatmentV2.UNPAID, AbsenceSalaryTreatmentV2.TO_CONFIRM)
                AlertDialog.Builder(context).setTitle("Traitement de l’absence").setItems(labels) { _, which -> treatment = values[which]; updateLabels() }.setNegativeButton("ANNULER", null).show()
            }
        ...
        if (type == AbsencePayrollImpactV2.TYPE_SICKNESS) {
            subrogationButton.setOnClickListener {
                ...
                AlertDialog.Builder(context).setTitle("Subrogation").setItems(labels) { _, which -> subrogation = values[which]; updateLabels() }.setNegativeButton("ANNULER", null).show()
            }
            providentButton.setOnClickListener {
                ...
                AlertDialog.Builder(context).setTitle("Prévoyance employeur pendant le maintien").setItems(labels) { _, which ->
                    when (which) {
                        0 -> { providentTreatment = AbsenceProvidentTreatmentV2.TO_CONFIRM; providentAmount = null; updateLabels() }
                        1 -> { providentTreatment = AbsenceProvidentTreatmentV2.NONE_CONFIRMED; providentAmount = null; updateLabels() }
        ...
        val box = LinearLayout(context).apply {
            orientation = VERTICAL; setPadding(dp(18), dp(8), dp(18), 0)
            addView(TextView(context).apply {
                ...
                textSize = 12f; setPadding(0, 0, 0, dp(8))
            })
        ...
        val dialog = AlertDialog.Builder(context).setTitle(AbsencePayrollImpactV2.label(type)).setView(box).setPositiveButton("ENREGISTRER", null).setNegativeButton("ANNULER", null).create()
        dialog.setOnShowListener {
            dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener {
                if (end.isBefore(start)) { Toast.makeText(context, "La date de fin doit être après le début", Toast.LENGTH_LONG).show(); return@setOnClickListener }
                if (providentTreatment == AbsenceProvidentTreatmentV2.NET_AMOUNT_CONFIRMED && providentAmount == null) {
                    Toast.makeText(context, "Renseigne le montant net de prévoyance", Toast.LENGTH_LONG).show(); return@setOnClickListener
                }
                val zone = ZoneId.systemDefault()
                V2RightsStore.upsertAbsence(context, AbsenceV2(
                    id = "absence-${UUID.randomUUID()}", employerId = companyId, type = type,
                    startMs = start.atStartOfDay(zone).toInstant().toEpochMilli(), endMs = end.plusDays(1).atStartOfDay(zone).toInstant().toEpochMilli(),
                    salaryTreatment = treatment, fullDay = true, status = DecisionStatusV2.CONFIRMED,
    ...
    private fun showProvidentAmountDialog(onConfirmed: (Double) -> Unit) {
        ...
        val dialog = AlertDialog.Builder(context)
            ...
            .setView(input).setPositiveButton("VALIDER", null).setNegativeButton("ANNULER", null).create()
        dialog.setOnShowListener {
            dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener {
                val value = parse(input.text.toString())
                if (value == null || value < 0.0) { input.error = "Montant net valide requis"; return@setOnClickListener }
                onConfirmed(value); dialog.dismiss()
            }
        }
        dialog.show()
    }
    ...
    private fun providentLabel(absence: AbsenceV2): String = when (absence.providentTreatment) {
        ...
        AbsenceProvidentTreatmentV2.NET_AMOUNT_CONFIRMED -> absence.employerProvidentOverlapNetAmount?.let { String.format(Locale.FRANCE, "%.2f € net confirmé", it) } ?: "montant à confirmer"
    }

    private fun confirmDeleteAbsence(absence: AbsenceV2, start: LocalDate, end: LocalDate) {
        AlertDialog.Builder(context).setTitle("Supprimer cette absence ?")
            .setMessage("${AbsencePayrollImpactV2.label(absence.type)}\n${start.format(dateFormat)} → ${end.format(dateFormat)}")
            .setNegativeButton("ANNULER", null).setPositiveButton("SUPPRIMER") { _, _ -> V2RightsStore.removeAbsence(context, absence.id); refresh() }.show()
    }

    private fun showCounterDialog() {
        fun amount(h: String) = EditText(context).apply {
            hint = h; inputType = InputType.TYPE_CLASS_NUMBER or InputType.TYPE_NUMBER_FLAG_DECIMAL or InputType.TYPE_NUMBER_FLAG_SIGNED; isSingleLine = true
        }
        ...
        val acquired = amount("Acquis — facultatif"); val available = amount("Disponible — facultatif"); val taken = amount("Pris — facultatif"); val anticipated = amount("Anticipé — facultatif"); val remaining = amount("Restant — facultatif")
        val box = LinearLayout(context).apply { orientation = VERTICAL; setPadding(dp(18), dp(8), dp(18), 0); listOf(label, unit, acquired, available, taken, anticipated, remaining).forEach(::addView) }
        val dialog = AlertDialog.Builder(context).setTitle("Compteur de droits").setMessage("HoraTrack enregistre les valeurs pour cette entreprise. La période de référence reste non renseignée sans source fiable.").setView(box).setPositiveButton("ENREGISTRER", null).setNegativeButton("ANNULER", null).create()
        dialog.setOnShowListener {
            dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener {
                ...
                if (values.all { it == null }) { Toast.makeText(context, "Renseigne au moins une valeur", Toast.LENGTH_LONG).show(); return@setOnClickListener }
                V2RightsStore.upsert(context, V2RightsStore.Balance("manual-${UUID.randomUUID()}", name, values[0], values[1], values[2], values[3], values[4], unit.text.toString().trim().ifBlank { "jours" }, 0L, Long.MAX_VALUE, "MANUAL", companyId))
                refresh(); dialog.dismiss()
            }
        }
        dialog.show()
    ...
    private fun number(v: Double) = String.format(Locale.FRANCE, "%.2f", v).trimEnd('0').trimEnd(',')
    private fun duration(ms: Long): String { val m = ms.coerceAtLeast(0L) / 60_000L; return String.format(Locale.FRANCE, "%dh%02d", m / 60L, m % 60L) }
