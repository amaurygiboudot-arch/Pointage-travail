...
open class LightReactiveJewelButton @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = android.R.attr.buttonStyle) : Button(context, attrs, defStyleAttr) {
    ...
    private val framePaint=Paint(Paint.ANTI_ALIAS_FLAG); private val iconPaint=Paint(Paint.ANTI_ALIAS_FLAG); private val detailPaint=Paint(Paint.ANTI_ALIAS_FLAG); private val pauseGlyphPaint=Paint(Paint.ANTI_ALIAS_FLAG)
    private var backgroundLayer:Bitmap?=null; private var sharpenedLayer:Bitmap?=null; private var loadedForId=-1; private var innerPressScale=1f
    protected var jewelLightAngle=-55f; protected var jewelAccent=Color.parseColor("#D6A84B"); protected var jewelAccentLight=Color.parseColor("#F3D58A"); private var nightLight=false
    ...
    open fun setLightAngle(a:Float){val n=((a%360)+360)%360;if(kotlin.math.abs(shortestDelta(jewelLightAngle,n))<.6)return;jewelLightAngle=n;invalidate()}
    open fun setJewelAccent(c:Int,l:Int){jewelAccent=c;jewelAccentLight=l;invalidate()}; open fun setNightLight(e:Boolean){if(nightLight==e)return;nightLight=e;invalidate()}
     ...
     * Aucune ligne/facette géométrique n'est ajoutée : les arêtes restent exactement celles du visuel original. */
    ...
    override fun onDraw(canvas:Canvas){ensureLayers();val w=width.toFloat();val h=height.toFloat();if(w<=0||h<=0)return;val save=canvas.save();canvas.scale(.5f,.5f);val rw=w*2;val rh=h*2;val radius=min(rw,rh)*.5f;val cx=rw*.5f;val cy=rh*.5f;drawFrame(canvas,cx,cy,radius);val ins=canvas.save();canvas.scale(innerPressScale,innerPressScale,cx,cy);val jr=radius*.885f;val dst=RectF(cx-jr,cy-jr,cx+jr,cy+jr)
        sharpenedLayer?.let{val contrast=1.18f;val o=-128*contrast+128;bitmapPaint.colorFilter=ColorMatrixColorFilter(ColorMatrix(floatArrayOf(contrast,0f,0f,0f,o,0f,contrast,0f,0f,o,0f,0f,contrast,0f,o,0f,0f,0f,1f,0f)));canvas.drawBitmap(it,null,dst,bitmapPaint);bitmapPaint.colorFilter=null}
        when(id){R.id.entryButton->drawIcon(canvas,cx,cy,jr,false);R.id.exitButton->drawIcon(canvas,cx,cy,jr,true);R.id.pauseButton->drawPause(canvas,cx,cy,jr)};canvas.restoreToCount(ins);canvas.restoreToCount(save);super.onDraw(canvas)}
