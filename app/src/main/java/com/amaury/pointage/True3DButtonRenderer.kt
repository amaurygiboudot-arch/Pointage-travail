...
class True3DButtonTextureView @JvmOverloads constructor(
    ...
    private class CrystalMeshRenderer(private val quality: DiamondQuality) {
        ...
        private fun buildMesh(width: Int, height: Int) {
            ...
            // Source de vérité : une entrée = une vraie surface fermée = un ID.
            val faces = ArrayList<TopologyFace>()
            ...
            for (i in 0 until 8) {
                ...
                faces += TopologyFace(
                    ...
                    listOf(table[i], table[n], innerCrown[n], innerCrown[i])
                )
            }
            for (i in 0 until 8) {
                val n = (i + 1) % 8
                faces += TopologyFace(
                    ...
                    listOf(innerCrown[i], innerCrown[n], outerCrown[n], outerCrown[i])
                )
            }
            for (i in 0 until 8) {
                val g0 = 2 * i
                val g1 = 2 * i + 1
                val g2 = (2 * i + 2) % 16
                faces += TopologyFace(DiamondFacetRegion.CROWN_OUTER, listOf(outerCrown[i], girdleTop[g0], girdleTop[g1]))
                faces += TopologyFace(DiamondFacetRegion.CROWN_OUTER, listOf(outerCrown[i], girdleTop[g1], girdleTop[g2]))
            }

            // La ceinture devient une vraie famille de facettes, avec IDs et mémoire.
            for (i in 0 until 16) {
                val n = (i + 1) % 16
                faces += TopologyFace(
                    DiamondFacetRegion.GIRDLE,
                    listOf(girdleTop[i], girdleTop[n], girdleBottom[n], girdleBottom[i])
                )
            }
            ...
            for (i in 0 until 8) {
                ...
                val g1 = 2 * i + 1
                val g2 = (2 * i + 2) % 16
                faces += TopologyFace(DiamondFacetRegion.PAVILION_UPPER, listOf(girdleBottom[g0], girdleBottom[g1], pavilionBreak[i]))
                faces += TopologyFace(DiamondFacetRegion.PAVILION_UPPER, listOf(girdleBottom[g1], girdleBottom[g2], pavilionBreak[i]))
            }
            for (i in 0 until 8) {
                val n = (i + 1) % 8
                faces += TopologyFace(
                    DiamondFacetRegion.PAVILION_LOWER,
                    listOf(pavilionBreak[i], pavilionBreak[n], culet)
                )
            }

            facetMemory.resetTopology(faces.size)

            val data = ArrayList<Float>(faces.size * 28)
            ...
            faces.forEachIndexed { id, face ->
                check(face.vertices.size >= 3) { "Facet $id has fewer than three vertices" }
                val n = normal(face.vertices[0], face.vertices[1], face.vertices[2])
                facetMemory.defineFacet(id, face.region, n[0], n[1], n[2])

                // Fan triangulation is internal only: every triangle keeps the same face ID/normal.
                for (i in 1 until face.vertices.lastIndex) {
                    emitTriangle(face.vertices[0], face.vertices[i], face.vertices[i + 1], n, id.toFloat())
                }

                // Only polygon boundaries become visible edges. No triangulation diagonal is emitted.
...
object True3DButtonInstaller {
    private const val TAG = "hp_true_3d_wrapped_v11_topology_driven"
    ...
    fun updateLight(root: View, lightAngle: Float) {
        hosts.entries.toList().forEach { (b, h) ->
            if (b.rootView === root.rootView) h.setLightAngle(lightAngle)
        }
    }

    private fun collect(v: View, out: MutableList<Button>) {
        if (v is Button && v.getTag(R.id.true3d_internal_tag) != TAG && !isPrimaryPointageButton(v)) {
            out.add(v)
        }
