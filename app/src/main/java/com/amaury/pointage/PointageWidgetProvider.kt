...
class PointageWidgetProvider : AppWidgetProvider() {
    ...
    companion object {
        ...
        private fun updateWidget(context: Context, manager: AppWidgetManager, widgetId: Int) {
            ...
            var accent = parseColor(stylePrefs.getString("widget_accent", null), "#D6A84B")
            val primaryText = AppearanceManager.bestTextColor(widgetBg)
            if (AppearanceManager.contrastRatio(accent, widgetBg) < 4.5) accent = primaryText
            ...
            views.setTextColor(R.id.widget_duration, primaryText)
            views.setTextColor(R.id.widget_entry_time, primaryText)
            views.setTextColor(R.id.widget_exit_time, primaryText)
            ...
            var entryText = "--:--"; var exitText = "--:--"; var durationText = "00h 00m"
            var stateText = "● HORS TRAVAIL"; var stateColor = primaryText; var locationText = "📍 Aucune zone"
            if (data.length() > 0) {
                ...
                if (zoneAddress.isNotEmpty()) locationText = "📍 ${shortLocation(zoneAddress)}" else if (last.isNull("exit")) locationText = "📍 Pointage manuel"
                if (last.isNull("exit")) {
                    durationText = formatDuration(System.currentTimeMillis() - entry)
                    stateText = "● EN COURS"
                    val green = Color.parseColor("#54D66A")
                    stateColor = if (AppearanceManager.contrastRatio(green, widgetBg) >= 4.5) green else primaryText
                } else {
                    val exit = last.getLong("exit")
                    exitText = formatTime(exit); durationText = formatDuration(exit - entry); stateText = "● TERMINÉ"
                    val red = Color.parseColor("#FF6B6B")
                    stateColor = if (AppearanceManager.contrastRatio(red, widgetBg) >= 4.5) red else primaryText
                }
            }
            views.setTextViewText(R.id.widget_entry_time, entryText); views.setTextViewText(R.id.widget_exit_time, exitText)
            views.setTextViewText(R.id.widget_duration, durationText); views.setTextViewText(R.id.widget_status, "HP V8")
            views.setTextViewText(R.id.widget_state, stateText); views.setTextViewText(R.id.widget_location, locationText)
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        when (intent.action) {
            ACTION_ENTRY -> { if (PointageStore.entry(context)) Toast.makeText(context, "Entrée enregistrée", Toast.LENGTH_SHORT).show() else Toast.makeText(context, "Une entrée est déjà en cours", Toast.LENGTH_SHORT).show(); updateAll(context) }
            ACTION_EXIT -> { if (PointageStore.exit(context)) Toast.makeText(context, "Sortie enregistrée", Toast.LENGTH_SHORT).show() else Toast.makeText(context, "Aucune entrée en cours", Toast.LENGTH_SHORT).show(); updateAll(context) }
        }
    }
}
