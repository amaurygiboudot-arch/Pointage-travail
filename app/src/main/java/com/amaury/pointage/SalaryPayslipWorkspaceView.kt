...
class SalaryPayslipWorkspaceView(context:Context,private val company:SalaryCompanyStore.Company):LinearLayout(context){
 ...
 private fun renderEstimate(){
  ...
  if(calc==null){add(TextView(context).apply{text="Calcul détaillé indisponible : complète le contrat et la convention de cette entreprise. HoraTrack n’invente aucune majoration.";textSize=14f})}else{
   ...
   val payroll=NetSalaryEngineV2.calculate(calc.monthlyEstimatedGross,year,CompanyPayrollOverridesV2.load(context,company.id))
