...
class V2PayslipImportActivity : Activity() {
    companion object { private const val REQUEST_FILE = 9601 }
    ...
    private fun askConfirmedValues(uri: Uri, mime: String?) {
        ...
        val monthButton = android.widget.Button(this).apply {
            text = SimpleDateFormat("MMMM yyyy", Locale.FRANCE).format(month.time).replaceFirstChar { it.uppercase() }
            isAllCaps = false
            setBackgroundResource(R.drawable.hp_panel)
        }
        val gross = EditText(this).apply {
            hint = "Brut du bulletin (€)"
            inputType = InputType.TYPE_CLASS_NUMBER or InputType.TYPE_NUMBER_FLAG_DECIMAL
        }
        val net = EditText(this).apply {
            hint = "Net du bulletin (€) — facultatif"
            inputType = InputType.TYPE_CLASS_NUMBER or InputType.TYPE_NUMBER_FLAG_DECIMAL
        }
        val box = LinearLayout(this).apply {
            orientation = LinearLayout.VERTICAL
            val d = resources.displayMetrics.density
            setPadding((18*d).toInt(), (8*d).toInt(), (18*d).toInt(), 0)
            addView(monthButton)
            addView(gross)
            addView(net)
        }
        ...
        val dialog = AlertDialog.Builder(this)
            .setTitle("Contrôle du bulletin")
            .setMessage("Le document est importé. Confirme les montants visibles sur le bulletin : HoraTrack ne doit pas inventer une extraction incertaine.")
            .setView(box)
        ...
        dialog.setOnShowListener {
            dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener {
                val grossValue = gross.text.toString().trim().replace(',', '.').toDoubleOrNull()
                val netValue = net.text.toString().trim().replace(',', '.').toDoubleOrNull()
    ...
    private fun chooseMonth(selected: Calendar, button: android.widget.Button) {
        ...
        AlertDialog.Builder(this)
            .setTitle("Période du bulletin")
            .setItems(labels.toTypedArray()) { _, which ->
                selected.timeInMillis = months[which].timeInMillis
