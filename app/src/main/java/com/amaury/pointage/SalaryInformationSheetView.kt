...
/**
 * Fiche de renseignements centrale de l'onglet Salaire.
 *
 * Elle réutilise volontairement salary_settings afin de préserver les données
 * déjà saisies dans l'application. Aucun reset et aucune copie parallèle.
 */
class SalaryInformationSheetView @JvmOverloads constructor(
    ...
    fun refresh() {
        companyName.setText(prefs.getString("company_name", "").orEmpty())
        companySiret.setText(prefs.getString("company_siret", "").orEmpty())
    ...
    private fun buildUi() {
        ...
        addView(TextView(context).apply {
            text = "Ces informations servent aux calculs de salaire et aux règles V2. Les données déjà enregistrées sont conservées et préremplies."
            textSize = 13f
        ...
        addLabel("CONTRAT")
        contractType.adapter = ArrayAdapter(
            context,
            android.R.layout.simple_spinner_dropdown_item,
            listOf("Choisir le type de contrat", "Temps plein", "Temps partiel", "Forfait", "Autre")
        )
        ...
        addView(save, LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, dp(54)).apply { topMargin = dp(5) })
    }
    ...
    private fun save() {
        ...
        if (weekly == null || weekly <= 0.0 || weekly > 80.0) {
        ...
        val editor = prefs.edit()
            ...
            .putString("contract_weekly_hours", weekly.toString())
    ...
    private fun updateConventionLabel() {
        val idcc = prefs.getString("company_idcc", "").orEmpty()
            .ifBlank { prefs.getString("convention_idcc", "").orEmpty() }
        val convention = ConventionCatalog.findByIdcc(context, idcc)
        conventionButton.text = convention?.displayName ?: "CHOISIR LA CONVENTION COLLECTIVE"
    }

    private fun updateStatus() {
        val missing = mutableListOf<String>()
        if (prefs.getString("company_name", "").orEmpty().isBlank() && prefs.getString("company_siret", "").orEmpty().isBlank()) missing += "entreprise"
        if (prefs.getString("contract_type", "").orEmpty().isBlank()) missing += "type de contrat"
        if (prefs.getString("contract_weekly_hours", "").orEmpty().toDoubleOrNull() == null) missing += "durée hebdomadaire"
        if (prefs.getString("hourly_rate", "").orEmpty().toDoubleOrNull() == null) missing += "taux horaire"
        status.text = if (missing.isEmpty()) "✅ Fiche complète pour les calculs principaux" else "⚠️ À compléter : ${missing.joinToString(", ")}"
    }

    private fun field(hintText: String, type: Int = InputType.TYPE_CLASS_TEXT) = EditText(context).apply {
        hint = hintText
        inputType = type
        isSingleLine = true
        textSize = 14f
        background = GradientDrawable().apply {
            ...
            setStroke(dp(1), Color.parseColor("#D6A84B"))
    ...
    private fun contractIndex(value: String) = when (value.uppercase()) {
        "FULL_TIME" -> 1
        "PART_TIME" -> 2
        "FORFAIT" -> 3
        "OTHER" -> 4
    ...
    private fun contractValue(index: Int) = when (index) {
        1 -> "FULL_TIME"
        2 -> "PART_TIME"
        3 -> "FORFAIT"
        4 -> "OTHER"
