...
object CompanyPauseAlarmManager {
    ...
    private fun scheduleOne(context: Context, company: Int, pauseIndex: Int, minuteOfDay: Int) {
        ...
        val whenCal = Calendar.getInstance(Locale.FRANCE).apply {
            set(Calendar.HOUR_OF_DAY, minuteOfDay / 60); set(Calendar.MINUTE, minuteOfDay % 60); set(Calendar.SECOND, 0); set(Calendar.MILLISECOND, 0)
            if (timeInMillis <= System.currentTimeMillis()) add(Calendar.DAY_OF_YEAR, 1)
        }
        val pi = pending(context, company, pauseIndex)
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S && !alarm.canScheduleExactAlarms()) alarm.setAndAllowWhileIdle(AlarmManager.RTC_WAKEUP, whenCal.timeInMillis, pi)
        else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) alarm.setExactAndAllowWhileIdle(AlarmManager.RTC_WAKEUP, whenCal.timeInMillis, pi)
    ...
    private fun pending(context: Context, company: Int, pauseIndex: Int): PendingIntent = PendingIntent.getBroadcast(
        context, 5200 + company * 10 + pauseIndex,
...
class CompanyPauseAlarmReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (company !in 1..2 || pauseIndex !in 1..2 || CompanyPauseAlarmManager.activeCompanySlot(context) != company || !CompanyBasePauseSettings.alarmEnabled(context, company, pauseIndex)) {
            CompanyPauseAlarmManager.scheduleAll(context); return
        }
        val type = when (CompanyBasePauseSettings.alarmSound(context, company, pauseIndex)) { "ringtone" -> RingtoneManager.TYPE_RINGTONE; "notification" -> RingtoneManager.TYPE_NOTIFICATION; else -> RingtoneManager.TYPE_ALARM }
        val ringtone = runCatching { RingtoneManager.getRingtone(context.applicationContext, RingtoneManager.getDefaultUri(type)) }.getOrNull()
        if (ringtone != null) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) ringtone.audioAttributes = AudioAttributes.Builder().setUsage(AudioAttributes.USAGE_ALARM).setContentType(AudioAttributes.CONTENT_TYPE_SONIFICATION).build()
