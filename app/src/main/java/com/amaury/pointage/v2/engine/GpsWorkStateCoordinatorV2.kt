...
object GpsWorkStateCoordinatorV2 {
    ...
    enum class Action { IGNORED, ENTRY_STARTED, RETURNED_TO_POSTE, EXIT_PENDING_CONFIRMATION, AMBIGUOUS_PENDING_CONFIRMATION, NO_CHANGE }
    data class Outcome(val action: Action, val requiresConfirmation: Boolean, val reason: String)
    data class Pending(val id: String, val atMs: Long, val placeId: String, val pointType: GpsPointTypeV2, val transition: GpsTransitionV2, val kind: Kind) {
        enum class Kind { EXIT_WORKSITE, AMBIGUOUS }
    }

    fun route(context: Context, event: GpsEventV2, decision: GpsDecisionV2): Outcome {
        if (!decision.accepted || decision.duplicate) return Outcome(Action.IGNORED, false, decision.reason)
        ...
        if (event.pointType == GpsPointTypeV2.POSTE && event.transition == GpsTransitionV2.ENTER) {
            ...
            if (pending?.kind == Pending.Kind.EXIT_WORKSITE && pending.placeId == event.placeId && event.atMs >= pending.atMs && event.atMs - pending.atMs <= RETURN_WINDOW_MS) {
            ...
            if (current == null || current.realExitMs != null) {
                val started = V2RuntimeStore.entry(
                    context = context,
                    nowMs = event.atMs,
                    placeId = event.placeId,
                    placeLabel = resolvePlaceLabel(context, event.placeId)
                )
                return Outcome(if (started) Action.ENTRY_STARTED else Action.NO_CHANGE, false, if (started) "Arrivée poste confirmée par GPS" else "Session déjà ouverte")
            }
        ...
        if (event.pointType == GpsPointTypeV2.POSTE && event.transition == GpsTransitionV2.EXIT) {
            if (current == null || current.realExitMs != null) return Outcome(Action.NO_CHANGE, false, "Aucune session V2 ouverte à terminer")
    ...
    fun pending(context: Context): Pending? {
        ...
        val transition = runCatching { GpsTransitionV2.valueOf(prefs.getString(KEY_PENDING_TRANSITION, null).orEmpty()) }.getOrNull() ?: return null
        return Pending(id, at, place, pointType, transition, kind)
    }

    fun shouldPrompt(context: Context, pending: Pending): Boolean = context.applicationContext.getSharedPreferences(PREFS, Context.MODE_PRIVATE).getString(KEY_PROMPTED_ID, null) != pending.id
    fun markPromptShown(context: Context, pending: Pending) { context.applicationContext.getSharedPreferences(PREFS, Context.MODE_PRIVATE).edit().putString(KEY_PROMPTED_ID, pending.id).apply() }
    fun allowPromptAgain(context: Context, pending: Pending) {
        val prefs = context.applicationContext.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
        if (prefs.getString(KEY_PENDING_ID, null) == pending.id) prefs.edit().remove(KEY_PROMPTED_ID).apply()
    }
    ...
    private fun resolvePlaceLabel(context: Context, placeId: String): String? {
        val raw = context.applicationContext.getSharedPreferences("gps_settings", Context.MODE_PRIVATE).getString("zones", "[]") ?: "[]"
        val zones = runCatching { JSONArray(raw) }.getOrElse { JSONArray() }
        for (i in 0 until zones.length()) {
            val zone = zones.optJSONObject(i) ?: continue
            if (zone.optString("id") != placeId) continue
            return zone.optString("address").trim().takeIf { it.isNotBlank() }
        }
        return null
    }
    ...
    private fun savePending(context: Context, event: GpsEventV2, kind: Pending.Kind) {
        context.applicationContext.getSharedPreferences(PREFS, Context.MODE_PRIVATE).edit()
            .putString(KEY_PENDING_ID, event.id).putLong(KEY_PENDING_AT, event.atMs).putString(KEY_PENDING_PLACE, event.placeId)
            .putString(KEY_PENDING_TYPE, "${event.pointType.name}:${kind.name}").putString(KEY_PENDING_TRANSITION, event.transition.name)
            .remove(KEY_PROMPTED_ID).apply()
    }
    private fun clearPending(context: Context) { context.applicationContext.getSharedPreferences(PREFS, Context.MODE_PRIVATE).edit().clear().apply() }
}
