...
object V2PayslipStore {
 ...
 fun comparison(context:Context,record:Record):PayslipComparisonV2?{val profile=V2ProfileStore.load(context,1);val rate=profile.contract?.grossHourlyRate?:return null;val idcc=profile.employer?.collectiveAgreementId.orEmpty();val convention=ConventionCatalog.findByIdcc(idcc)?:ConventionCatalog.conventions.firstOrNull{it.idcc.isBlank()}?:return null;val expected=V2SalaryAdapter.calculate(context,record.year,record.month,rate,convention);if(expected.completedSessions==0&&expected.warnings.isNotEmpty())return null;val observed=linkedMapOf<String,Double>();record.gross?.let{observed["Brut"]=it};if(observed.isEmpty())return null;return PayslipEngineV2.compare(mapOf("Brut" to expected.monthlyEstimatedGross),observed,0.02)}
