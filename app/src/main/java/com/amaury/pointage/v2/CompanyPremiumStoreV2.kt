...
object CompanyPremiumStoreV2 {
    ...
    fun resolve(context: Context, companyId: String, period: YearMonth): CompanyPremiumResolverV2.Snapshot =
        CompanyPremiumResolverV2.resolve(list(context, companyId), period)
    ...
    private fun fromJson(o: JSONObject?): CompanyPremiumResolverV2.Record? {
        ...
        val kind = runCatching { CompanyPremiumResolverV2.Kind.valueOf(o.optString("kind")) }.getOrNull() ?: return null
        fun month(key: String): YearMonth? = o.optString(key).takeIf { it.isNotBlank() && it != "null" }?.let {
            runCatching { YearMonth.parse(it) }.getOrNull()
        }
        return CompanyPremiumResolverV2.Record(
            id = id,
            label = label,
            grossAmount = amount,
            kind = kind,
