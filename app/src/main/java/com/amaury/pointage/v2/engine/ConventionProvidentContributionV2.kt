...
object ConventionProvidentContributionV2 {
    /**
     * EXACT : les taux salarié/employeur sont imposés par la règle vérifiée.
     * DEFAULT_MODIFIABLE_BY_COMPANY_AGREEMENT : les deux taux sont seulement la répartition
     * conventionnelle par défaut ; l'entreprise peut avoir une répartition différente.
     */
    enum class AllocationRule {
        EXACT,
        DEFAULT_MODIFIABLE_BY_COMPANY_AGREEMENT
    }
    ...
    data class Band(
        ...
        /** Taux exact ou taux salarié par défaut selon [allocationRule]. */
        val employeeRate: Double,
        /** Taux exact ou taux employeur par défaut selon [allocationRule]. */
        val employerRate: Double,
        /** Minimum global obligatoire lorsque la convention exprime seulement un plancher. */
        val minimumTotalRate: Double? = null,
        /** Minimum employeur obligatoire lorsque la convention exprime seulement un plancher. */
        val minimumEmployerRate: Double? = null,
        val allocationRule: AllocationRule = AllocationRule.EXACT
    ) {
        fun structurallyValid(): Boolean {
            if (label.isBlank()) return false
            if (!lowerCeilingMultiple.isFinite() || lowerCeilingMultiple < 0.0) return false
            if (upperCeilingMultiple != null && (!upperCeilingMultiple.isFinite() || upperCeilingMultiple <= lowerCeilingMultiple)) return false
            if (!employeeRate.isFinite() || employeeRate !in 0.0..1.0) return false
            if (!employerRate.isFinite() || employerRate !in 0.0..1.0) return false
            if (employeeRate <= 0.0 && employerRate <= 0.0) return false

            val minTotal = minimumTotalRate
            val minEmployer = minimumEmployerRate
            if (minTotal != null && (!minTotal.isFinite() || minTotal !in 0.0..1.0 || minTotal <= 0.0)) return false
            if (minEmployer != null && (!minEmployer.isFinite() || minEmployer !in 0.0..1.0 || minEmployer < 0.0)) return false

            return when (allocationRule) {
                AllocationRule.EXACT -> minTotal == null && minEmployer == null
                AllocationRule.DEFAULT_MODIFIABLE_BY_COMPANY_AGREEMENT -> {
                    if (minTotal == null || minEmployer == null) return false
                    if (minEmployer > minTotal) return false
                    val defaultTotal = employeeRate + employerRate
                    defaultTotal + RATE_EPSILON >= minTotal && employerRate + RATE_EPSILON >= minEmployer
                }
            }
        }
        ...
        val needsCompanyAllocationConfirmation: Boolean
            get() = allocationRule == AllocationRule.DEFAULT_MODIFIABLE_BY_COMPANY_AGREEMENT
    }
    ...
    fun calculate(
        ...
        /**
         * Vrai uniquement si l'entreprise a confirmé que la répartition conventionnelle par défaut
         * est bien celle applicable au salarié (absence d'accord d'entreprise dérogatoire ou preuve
         * équivalente). Ce booléen ne doit jamais être déduit du seul texte de branche.
         */
        ...
        if (tier.bands.any { it.needsCompanyAllocationConfirmation } && !companyDefaultAllocationConfirmed) {
            return unavailable(
                "Prévoyance conventionnelle IDCC $normalized : la convention fixe des minima et une répartition par défaut modifiable par accord d'entreprise ; la retenue salariale exacte est à confirmer avant calcul. Source : ${selected.source}."
        ...
        val lines = tier.bands.sortedBy { it.lowerCeilingMultiple }.mapNotNull { band ->
            ...
            if (base <= 0.0) null else AppliedBand(
                ...
                minimumTotalRate = band.minimumTotalRate,
                minimumEmployerRate = band.minimumEmployerRate,
                allocationRule = band.allocationRule
            )
        }

        val usesConfirmedDefaultAllocation = tier.bands.any { it.needsCompanyAllocationConfirmation }
        return Result(
            applicable = true,
            eligibilityConfirmed = true,
            reliable = true,
            ...
            warnings = listOf(
                if (usesConfirmedDefaultAllocation) {
                    "Prévoyance conventionnelle : répartition par défaut appliquée uniquement après confirmation de son applicabilité dans l'entreprise. Source : ${selected.source}."
                } else {
                    "Prévoyance conventionnelle : cotisation calculée uniquement à partir du barème vérifié. Source : ${selected.source}."
                }
            )
        )
    }
