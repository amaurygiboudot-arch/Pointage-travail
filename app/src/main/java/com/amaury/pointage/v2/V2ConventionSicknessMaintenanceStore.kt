...
object V2ConventionSicknessMaintenanceStore {
    ...
    private const val STORAGE_WARNING =
        "KALI maintien maladie : historique local des barèmes conventionnels incohérent ; aucun droit maladie ne peut être déduit de ce stockage."

    data class ReadResult(
        val rules: List<ConventionSicknessMaintenanceV2.Rule>,
    ...
    fun saveConfirmed(context: Context, rule: ConventionSicknessMaintenanceV2.Rule) {
        ...
        check(current.all { it.structurallyValid() }) { "KALI maintien maladie : historique conventionnel invalide." }
        check(!hasDuplicateRuleIds(current)) { "KALI maintien maladie : identifiant de règle dupliqué." }
        persist(context, current)
    }

    private fun persist(context: Context, rules: List<ConventionSicknessMaintenanceV2.Rule>) {
        check(rules.all { it.structurallyValid() }) { "KALI maintien maladie : historique conventionnel invalide." }
        check(!hasDuplicateRuleIds(rules)) { "KALI maintien maladie : identifiant de règle dupliqué." }
        val array = JSONArray()
        ...
        check(saved) { "KALI maintien maladie : stockage local des barèmes impossible." }
    }

    internal fun decodeConfirmed(raw: String): ReadResult {
        val array = runCatching { JSONArray(raw) }.getOrNull()
            ?: return ReadResult(emptyList(), false, listOf(STORAGE_WARNING))
        val rules = mutableListOf<ConventionSicknessMaintenanceV2.Rule>()
        var malformed = false
    ...
    private fun hasDuplicateRuleIds(rules: List<ConventionSicknessMaintenanceV2.Rule>): Boolean =
        rules.groupBy {
