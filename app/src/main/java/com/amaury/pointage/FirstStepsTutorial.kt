...
object FirstStepsTutorial {
    ...
    private val steps = listOf(
        Step("Bienvenue dans AGKGMG 👋", "AGKGMG démarre avec la CONFIGURATION INTELLIGENTE activée. Le but est simple : tu renseignes surtout ton entreprise, puis l'application apprend progressivement le lieu de travail et les habitudes de pause. Android te demandera toujours ton accord pour les autorisations de localisation."),
        Step("1 • Le SIRET lance la configuration", "Dans l'onglet Salaire, saisis le SIRET à 14 chiffres de ton entreprise puis appuie sur RECHERCHER. AGKGMG récupère automatiquement le nom, l'adresse de l'établissement et les informations disponibles. Dès que le SIRET est reconnu, le vrai nom de l'entreprise remplace Entreprise 1 ou Entreprise 2 dans l'interface, même si le SIRET a été saisi manuellement.", R.id.tabSalary),
        Step("2 • Active la détection intelligente", "AGKGMG peut détecter progressivement ton lieu de travail. Active cette fonction seulement si tu le souhaites : Android demandera les autorisations de localisation nécessaires et AGKGMG n'enregistrera jamais un nouveau lieu de travail sans confirmation.", R.id.tabSettings),
        Step("3 • Éviter de confondre travail et domicile", "Un lieu n'est proposé comme travail qu'après une présence d'au moins 7 heures pendant 3 jours consécutifs dans la même zone. Ensuite AGKGMG demande si c'est bien ton lieu de travail. Une même zone n'est proposée que deux fois maximum pour ne pas être intrusive.", R.id.tabSettings),
        Step("4 • Entreprises déjà renseignées", "Si aucun employeur n'est encore enregistré, le premier lieu confirmé est associé au premier emplacement disponible. Si Entreprise 1 est déjà renseignée et qu'un autre employeur est confirmé, AGKGMG utilise Entreprise 2. Si les deux sont déjà remplies, aucune donnée n'est écrasée.", R.id.tabSalary),
        ...
        Step("6 • Comprends le rayon GPS", "Le rayon est la distance autour du point précis dans laquelle AGKGMG considère que tu es sur le lieu de travail. Exemple : 150 m = un cercle de 150 m autour du point choisi. Un rayon trop grand peut déclencher trop tôt ; un rayon trop petit peut rater une arrivée à cause de la précision GPS.", R.id.tabSettings),
        ...
        Step("C'est prêt ✓", "Au quotidien, l'objectif est que tu aies le moins possible à régler : SIRET, autorisation GPS si tu actives la détection intelligente, puis AGKGMG apprend. Le nom réel de chaque entreprise est utilisé dès qu'il est connu.", R.id.tabToday)
    )
