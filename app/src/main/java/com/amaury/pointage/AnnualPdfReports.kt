...
/** Deux rapports annuels : temps de travail et rémunération estimée. */
object AnnualPdfReports {
    fun writeWork(context:Context,data:JSONArray,year:Int,out:OutputStream){
        val pdf=PdfDocument();val p=pdf.startPage(PdfDocument.PageInfo.Builder(595,842,1).create());val c=p.canvas
        val title=paint(20f,true);val h=paint(10f,true);val n=paint(9f,false);val fill=Paint().apply{color=Color.rgb(243,239,230)};var y=34f
        c.drawText("BILAN ANNUEL DU TEMPS DE TRAVAIL — $year",30f,y,title);y+=34;c.drawRect(30f,y,565f,y+24,fill)
        ...
        y+=8;c.drawRect(30f,y,565f,y+48,fill);c.drawText("TOTAL ANNÉE",38f,y+17,h);c.drawText("$yd jours  •  présence ${dur(pr)}  •  payé ${dur(paid)}  •  pauses équipe payées ${dur(pp)}  •  pauses déduites ${dur(up)}  •  $meals paniers",38f,y+36,h);pdf.finishPage(p);pdf.writeTo(out);pdf.close()
    }
    ...
    fun writeSalary(context:Context,data:JSONArray,year:Int,out:OutputStream){
        ...
        val pdf=PdfDocument();val p=pdf.startPage(PdfDocument.PageInfo.Builder(595,842,1).create());val c=p.canvas;val title=paint(19f,true);val h=paint(9.5f,true);val n=paint(8.8f,false);val fill=Paint().apply{color=Color.rgb(243,239,230)};var y=34f;c.drawText("ESTIMATION ANNUELLE DE RÉMUNÉRATION — $year",30f,y,title);y+=28;c.drawText("Document indicatif — ne remplace pas un bulletin de paie.",30f,y,n);y+=24;c.drawRect(30f,y,565f,y+24,fill);arrayOf("Mois","Heures payées","Heures sup.","Nuit","Paniers","Brut estimé").forEachIndexed{i,s->c.drawText(s,floatArrayOf(34f,145f,245f,335f,410f,485f)[i],y+16,h)};y+=30
        ...
        y+=8;c.drawRect(30f,y,565f,y+58,fill);c.drawText("TOTAL ANNÉE",38f,y+17,h);c.drawText("Temps payé : ${dur(annualPaid)}",38f,y+35,h);c.drawText("Paniers : ${euro.format(annualMeals)}   •   Brut annuel estimé : ${if(rate>0)euro.format(annualGross) else "taux horaire non renseigné"}",250f,y+35,h);y+=75;c.drawText("L'estimation utilise les paramètres Salaire, la convention connue, les majorations intégrées et les paniers enregistrés.",30f,y,n);pdf.finishPage(p);pdf.writeTo(out);pdf.close()
    }
    private fun paint(size:Float,bold:Boolean)=Paint(Paint.ANTI_ALIAS_FLAG).apply{color=Color.rgb(40,40,40);textSize=size;if(bold)typeface=Typeface.DEFAULT_BOLD}
