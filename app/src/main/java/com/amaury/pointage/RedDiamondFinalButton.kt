...
open class RedDiamondFinalButton @JvmOverloads constructor(
    ...
    private fun drawFacets(c:Canvas,cx:Float,cy:Float,r:Float){
        val ir=r*.30f;val mr=r*.64f;val orr=r*.96f
        ...
        for(i in 0 until FACETS){val a0=angle(i);val a1=angle(i+1);val am=(a0+a1)*.5f;val m0=pt(cx,cy,mr,a0);val m1=pt(cx,cy,mr,a1);val o0=pt(cx,cy,orr,a0);val o1=pt(cx,cy,orr,a1);val om=pt(cx,cy,orr,am);facet(c,poly(m0,o0,om,m1),i+5,(a0+am)*.5f,2,.74f);facet(c,poly(m1,om,o1),i+12,(am+a1)*.5f,2,.64f)}
    }
