...
class DiamondDesignerReferenceActivity : Activity() {
    ...
    private fun buildUi(): View {
        ...
        root.addView(buildHeader(),lpMatch(dp(54)));root.addView(buildToolbar(),lpMatch(dp(54)))

        // The canvas is deliberately outside every ScrollView: while a settings tab scrolls,
        // the design stays visible and can be judged live.
        ...
        root.addView(workspace,LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,0,0.43f).apply{setMargins(dp(10),dp(6),dp(10),dp(4))})

        val previewRow=LinearLayout(this).apply{orientation=LinearLayout.HORIZONTAL;gravity=Gravity.CENTER_VERTICAL;addView(smallButton("AVANT / APRÈS"){toggleBeforeAfter()},weightLp());addView(smallButton("RÉINITIALISER VUE"){resetSelectedView()},weightLp())}
        root.addView(card(previewRow),LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,dp(52)).apply{setMargins(dp(10),0,dp(10),dp(4))})

        // Only the tool area scrolls. Categories are separated so there is never one giant
        // vertical page between the user and the preview.
        val tools=buildToolTabs()
        root.addView(tools,LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,0,0.57f))
        return root
    }
    ...
    private fun buildHeader(): View = LinearLayout(this).apply {
        ...
        addView(TextView(this@DiamondDesignerReferenceActivity).apply{text="DIAMOND DESIGNER";setTextColor(Color.WHITE);textSize=20f;gravity=Gravity.CENTER_VERTICAL},LinearLayout.LayoutParams(0,ViewGroup.LayoutParams.MATCH_PARENT,1f))
    ...
    private fun buildToolbar(): View = HorizontalScrollView(this).apply {
        isHorizontalScrollBarEnabled=false;addView(LinearLayout(this@DiamondDesignerReferenceActivity).apply{orientation=LinearLayout.HORIZONTAL;gravity=Gravity.CENTER_VERTICAL;setPadding(dp(8),dp(3),dp(8),dp(3));addView(toolButton("▶ + ENTRÉE"){canvas.addEntryButton();refreshAll()});addView(toolButton("Ⅱ + PAUSE"){canvas.addPauseButton();refreshAll()});addView(toolButton("■ + SORTIE"){canvas.addExitButton();refreshAll()});addView(toolButton("＋ CADRE"){canvas.addFrame();refreshAll()});addView(toolButton("＋ FOND"){canvas.addBackground();refreshAll()});addView(toolButton("◇ BIBLIOTHÈQUE"){showLibrary()})})
    }

    private fun buildToolTabs(): View {
        val shell=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;setBackgroundColor(Color.parseColor("#0D1116"));setPadding(dp(8),0,dp(8),dp(6))}
        selectionLabel=TextView(this).apply{text="Aucun élément sélectionné";setTextColor(Color.parseColor("#D7E5F2"));textSize=13f;setPadding(dp(6),dp(4),dp(6),dp(4))};shell.addView(selectionLabel)

        val layers=panel{p->p.addView(subTitle("CALQUES"));layersHost=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL};p.addView(layersHost)}
        val form=panel{p->p.addView(subTitle("GÉNÉRAL"));rotation=addControl(p,"Rotation",0,3600);p.addView(subTitle("ANNEAUX & FACETTES"));ringCount=addControl(p,"Nombre d’anneaux",1,3);facetDensity=addControl(p,"Facettes par anneau",8,64);facetDepth=addControl(p,"Profondeur des facettes",0,1000);ring1=addControl(p,"Anneau 1 (16) — déformation",200,1800);ring2=addControl(p,"Anneau 2 (32) — déformation",200,1800);ring3=addControl(p,"Anneau 3 (32) — déformation",200,1800);p.addView(subTitle("DÉFORMATION GLOBALE"));lens=addControl(p,"Bombage (convexe)",0,1000);ellipseX=addControl(p,"Ellipticité X",500,1500);ellipseY=addControl(p,"Ellipticité Y",500,1500);p.addView(subTitle("TAILLE & POSITION"));sizeControl=addControl(p,"Taille du bouton",100,1000);posX=addControl(p,"Position X",0,1000);posY=addControl(p,"Position Y",0,1000);p.addView(wideButton("RÉINITIALISER FORME"){resetForm()})}
        val material=panel{p->p.addView(subTitle("MATIÈRE"));transparency=addControl(p,"Transparence (efface le bouton)",0,950);translucency=addControl(p,"Translucidité (laisse voir le fond)",0,1000);p.addView(help("Transparence = disparition globale. Translucidité = matière traversée tout en gardant les facettes lisibles."))}
        ...
        val lighting=panel{p->p.addView(subTitle("LUMIÈRE"));light=addControl(p,"Direction",0,3600);p.addView(help("Les reflets évoluent sans remplacer les couleurs permanentes des facettes."))}
        val presetsPanel=panel{p->p.addView(subTitle("PRÉRÉGLAGES"));presetsHost=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL};p.addView(presetsHost)}
        val exportPanel=panel{p->p.addView(subTitle("EXPORTATION"));p.addView(wideButton("▧ EXPORTER BOUTON PNG"){exportPng()});p.addView(wideButton("▤ EXPORTER THÈME (.JSON)"){exportJson()});p.addView(wideButton("▯ APERÇU DANS L’APP"){showAppPreview()})}
        ...
        val tabs=HorizontalScrollView(this).apply{isHorizontalScrollBarEnabled=false;addView(LinearLayout(this@DiamondDesignerReferenceActivity).apply{orientation=LinearLayout.HORIZONTAL;addView(tabButton("☷ CALQUES"){showPanel(layers)});addView(tabButton("◇ FORME"){showPanel(form)});addView(tabButton("◇ MATIÈRE"){showPanel(material)});addView(tabButton("◐ ARÊTES"){showPanel(edges)});addView(tabButton("☼ LUMIÈRE"){showPanel(lighting)});addView(tabButton("★ PRESETS"){showPanel(presetsPanel)});addView(tabButton("⇧ EXPORT"){showPanel(exportPanel)});addView(tabButton("♙ ASSISTANT"){showPanel(assistant)})})}
        shell.addView(tabs,lpMatch(dp(46)))
        panelHost=FrameLayout(this);listOf(layers,form,material,edges,lighting,presetsPanel,exportPanel,assistant).forEach{panelHost.addView(it)};shell.addView(panelHost,LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,0,1f));showPanel(form)
        return shell
    }

    private fun bindCanvas(){
        canvas.onSelectionChanged={e->if(!showingBefore)beforeState=e?.copy();updateControls(e);refreshLayers()};canvas.onDesignChanged={canvas.selectedElement()?.let(::updateLabels);refreshLayers()}
        lens.bar.listen{canvas.setSelectedLens(it/1000f)};ring1.bar.listen{canvas.setSelectedRingGain(1,it/1000f)};ring2.bar.listen{canvas.setSelectedRingGain(2,it/1000f)};ring3.bar.listen{canvas.setSelectedRingGain(3,it/1000f)};ringCount.bar.listen{canvas.setSelectedRingCount(it)};facetDensity.bar.listen{canvas.setSelectedFacetDensity(it)};facetDepth.bar.listen{canvas.setSelectedFacetDepth(it/1000f)}
        transparency.bar.listen{canvas.setSelectedTransparency(it/1000f)};translucency.bar.listen{canvas.setSelectedTranslucency(it/1000f)};rotation.bar.listen{canvas.setSelectedRotation(it/10f)};light.bar.listen{canvas.setSelectedLightAngle(it/10f)};edgeWidth.bar.listen{canvas.setSelectedEdgeWidth(it/100f)};edgeAlpha.bar.listen{canvas.setSelectedEdgeAlpha(it/1000f)};edgeContrast.bar.listen{canvas.setSelectedEdgeContrast(it/1000f)};edgeSoftness.bar.listen{canvas.setSelectedEdgeSoftness(it/1000f)};radialEdges.bar.listen{canvas.setSelectedRadialEdgeGain(it/1000f)};circularEdges.bar.listen{canvas.setSelectedCircularEdgeGain(it/1000f)}
        sizeControl.bar.listen{p->mutateSelected{e->val side=max(72f,min(canvas.width,canvas.height)*(p/1000f));val cx=e.x+e.width/2f;val cy=e.y+e.height/2f;e.width=side;e.height=side;e.x=cx-side/2f;e.y=cy-side/2f}};posX.bar.listen{p->mutateSelected{e->e.x=(canvas.width-e.width).coerceAtLeast(0f)*p/1000f}};posY.bar.listen{p->mutateSelected{e->e.y=(canvas.height-e.height).coerceAtLeast(0f)*p/1000f}};ellipseX.bar.listen{p->mutateSelected{e->val center=e.x+e.width/2f;val base=min(e.width,e.height);e.width=base*(p/1000f);e.x=center-e.width/2f}};ellipseY.bar.listen{p->mutateSelected{e->val center=e.y+e.height/2f;val base=min(e.width,e.height);e.height=base*(p/1000f);e.y=center-e.height/2f}}
        refreshAll()
    }
    ...
    private fun refreshLayers(){if(!::layersHost.isInitialized)return;layersHost.removeAllViews();val list=canvas.allElements();if(list.isEmpty()){layersHost.addView(help("Aucun calque"));return};list.asReversed().forEach{e->val row=LinearLayout(this).apply{orientation=LinearLayout.HORIZONTAL;gravity=Gravity.CENTER_VERTICAL};row.addView(smallButton(if(e.visible)"◉" else "○"){e.visible=!e.visible;canvas.invalidate();refreshLayers()},LinearLayout.LayoutParams(dp(52),dp(42)));row.addView(TextView(this).apply{text=e.name;setTextColor(if(canvas.selectedElement()===e)Color.parseColor("#E0B24E") else Color.WHITE);textSize=14f;gravity=Gravity.CENTER_VERTICAL;setOnClickListener{canvas.selectElement(e);refreshAll()}},LinearLayout.LayoutParams(0,dp(42),1f));row.addView(smallButton(if(e.locked)"🔒" else "⚙"){e.locked=!e.locked;canvas.invalidate();refreshLayers()},LinearLayout.LayoutParams(dp(58),dp(42)));layersHost.addView(row)};val s=canvas.selectedElement();if(isDiamond(s)&&s!=null){layersHost.addView(subTitle("SOUS-CALQUES DU DIAMANT"));subLayerRow("Anneau 3 (${s.facetDensity})",s.ring3Visible){canvas.setSelectedSubLayerVisible("ring3",it)};subLayerRow("Anneau 2 (${s.facetDensity})",s.ring2Visible){canvas.setSelectedSubLayerVisible("ring2",it)};subLayerRow("Anneau 1 (${max(8,s.facetDensity/2)})",s.ring1Visible){canvas.setSelectedSubLayerVisible("ring1",it)};subLayerRow("Noyau",s.coreVisible){canvas.setSelectedSubLayerVisible("core",it)};subLayerRow("Facettes",s.facetsVisible){canvas.setSelectedSubLayerVisible("facets",it)};subLayerRow("Arêtes",s.edgesVisible){canvas.setSelectedSubLayerVisible("edges",it)}}}
    ...
    private fun resetSelectedView(){mutateSelected{it.rotation=0f}};private fun resetForm(){mutateSelected{e->e.rotation=0f;e.lensStrength=.5f;e.ring1Gain=1f;e.ring2Gain=1f;e.ring3Gain=1f;e.ringCount=3;e.facetDensity=32;e.facetDepth=.68f;val side=min(canvas.width,canvas.height)*.42f;e.width=side;e.height=side;e.x=(canvas.width-side)/2f;e.y=(canvas.height-side)/2f}}
    ...
    private fun exportPng(){runCatching{DiamondDesignerExporter.exportCanvasPng(this,canvas,currentProjectName)}.onSuccess{shareFile(it,"image/png")}.onFailure{Toast.makeText(this,it.message?:"Erreur export PNG",Toast.LENGTH_LONG).show()}};private fun exportJson(){val project=DiamondDesignerProjectStore.Project(currentProjectName,canvas.snapshot());runCatching{DiamondDesignerExporter.exportProjectJson(this,project)}.onSuccess{shareFile(it,"application/json")}.onFailure{Toast.makeText(this,it.message?:"Erreur export JSON",Toast.LENGTH_LONG).show()}};private fun shareFile(file:File,mime:String){val uri=FileProvider.getUriForFile(this,"${BuildConfig.APPLICATION_ID}.exports",file);startActivity(Intent.createChooser(Intent(Intent.ACTION_SEND).apply{type=mime;putExtra(Intent.EXTRA_STREAM,uri);addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)},"Exporter avec…"))};private fun showAppPreview(){val bmp=DiamondDesignerExporter.renderBitmap(canvas);val image=ImageView(this).apply{setImageBitmap(bmp);adjustViewBounds=true;scaleType=ImageView.ScaleType.FIT_CENTER;setBackgroundColor(Color.parseColor("#07090C"))};AlertDialog.Builder(this).setTitle("Aperçu dans l’app").setView(image).setPositiveButton("Fermer"){_,_->bmp.recycle()}.setOnCancelListener{bmp.recycle()}.show()}
    ...
    private fun panel(build:(LinearLayout)->Unit):ScrollView{val body=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;setPadding(dp(8),dp(4),dp(8),dp(10))};build(body);return ScrollView(this).apply{visibility=View.GONE;isFillViewport=true;addView(body);setBackgroundColor(Color.parseColor("#0D1116"))}}
    ...
    private fun sectionTitle(text:String)=TextView(this).apply{this.text=text;setTextColor(Color.WHITE);textSize=15f;setPadding(dp(4),dp(10),dp(4),dp(4))};private fun subTitle(text:String)=TextView(this).apply{this.text=text;setTextColor(Color.WHITE);textSize=14f;setPadding(0,dp(8),0,dp(4))};private fun help(text:String)=TextView(this).apply{this.text=text;setTextColor(Color.parseColor("#91A4B5"));textSize=12f;setPadding(0,dp(4),0,dp(4))};private fun card(content:View)=FrameLayout(this).apply{setPadding(dp(6),dp(4),dp(6),dp(4));setBackgroundColor(Color.parseColor("#11161C"));addView(content)}
    private fun smallButton(text:String,action:()->Unit)=Button(this).apply{this.text=text;isAllCaps=false;textSize=10f;setOnClickListener{action()}};private fun wideButton(text:String,action:()->Unit)=Button(this).apply{this.text=text;isAllCaps=false;setOnClickListener{action()}};private fun toolButton(text:String,action:()->Unit)=Button(this).apply{this.text=text;isAllCaps=false;textSize=12f;setOnClickListener{action()}}.also{it.layoutParams=LinearLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,dp(44)).apply{marginEnd=dp(5)}};private fun tabButton(text:String,action:()->Unit)=Button(this).apply{this.text=text;isAllCaps=false;textSize=11f;setOnClickListener{action()}}.also{it.layoutParams=LinearLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,dp(42)).apply{marginEnd=dp(3)}}
    private fun lpMatch(h:Int)=LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,h);private fun weightLp()=LinearLayout.LayoutParams(0,dp(44),1f).apply{marginEnd=dp(4)};private fun dp(v:Int)=(v*resources.displayMetrics.density).toInt()
}
