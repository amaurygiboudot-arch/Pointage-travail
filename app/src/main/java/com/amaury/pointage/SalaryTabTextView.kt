...
class SalaryTabTextView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : TextView(context, attrs, defStyleAttr) {
    ...
    private fun installSalaryAutoHide() {
        ...
        fun scheduleCheck() { if (autoHidePosted) return; autoHidePosted = true; post { autoHidePosted = false; val salary=contentPanel.findViewWithTag<SalaryV2RootView>(SalaryV2RootView.TAG); val shift=root.findViewById<View>(R.id.shiftControlView); val other=root.findViewById<View>(R.id.pointageButtons)?.visibility==View.VISIBLE || root.findViewById<View>(R.id.historyText)?.visibility==View.VISIBLE || root.findViewById<View>(R.id.analyticsPdfPanel)?.visibility==View.VISIBLE || root.findViewById<View>(R.id.gpsSettingsPanel)?.visibility==View.VISIBLE; if(other){salary?.visibility=View.GONE;shift?.visibility=View.GONE}; syncSelectedTabFromVisiblePanel() } }
