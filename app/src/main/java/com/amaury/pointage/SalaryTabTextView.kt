...
class SalaryTabTextView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : TextView(context, attrs, defStyleAttr) {
    ...
    private fun showIntegratedSalaryTab() {
        ...
        val homePanel = root.findViewById<View>(R.id.celestialHomePanel)
        val sunIndicator = root.findViewById<SunIndicatorView>(R.id.sunIndicator)
        ...
        (context as? MainActivity)?.onSalaryTabShown()
            ?: context.getSharedPreferences("navigation_state", Context.MODE_PRIVATE)
                .edit()
                .putString("active_tab", "salary")
                .apply()

        // La navigation Salaire cible uniquement la racine V2. L'ancienne interface reste dans le code
        // comme filet de migration, sans être résolue ni pilotée depuis cette navigation.
    ...
    private fun installSalaryAutoHide() {
        ...
        fun scheduleCheck() { if (autoHidePosted) return; autoHidePosted = true; post { autoHidePosted = false; val salary=contentPanel.findViewWithTag<SalaryV2RootView>(SalaryV2RootView.TAG); val shift=root.findViewById<View>(R.id.shiftControlView); val other=root.findViewById<View>(R.id.celestialHomePanel)?.visibility==View.VISIBLE || root.findViewById<View>(R.id.pointageButtons)?.visibility==View.VISIBLE || root.findViewById<View>(R.id.historyText)?.visibility==View.VISIBLE || root.findViewById<View>(R.id.analyticsPdfPanel)?.visibility==View.VISIBLE || root.findViewById<View>(R.id.gpsSettingsPanel)?.visibility==View.VISIBLE; if(other){salary?.visibility=View.GONE;shift?.visibility=View.GONE}; syncSelectedTabFromVisiblePanel() } }
        listOf(root.findViewById<View>(R.id.celestialHomePanel),root.findViewById<View>(R.id.pointageButtons),root.findViewById<View>(R.id.historyText),root.findViewById<View>(R.id.analyticsPdfPanel),root.findViewById<View>(R.id.gpsSettingsPanel)).forEach{v->v?.addOnLayoutChangeListener{_,_,_,_,_,_,_,_,_->scheduleCheck()}}
    }
    private fun applyTabTypography(){listOf(R.id.tabHome,R.id.tabToday,R.id.tabHistory,R.id.tabAnalytics,R.id.tabSalary,R.id.tabSettings).forEach{id->rootView.findViewById<TextView>(id)?.apply{textSize=10.5f;typeface=Typeface.create("sans-serif-condensed",Typeface.BOLD);maxLines=2;ellipsize=TextUtils.TruncateAt.END;includeFontPadding=false;gravity=Gravity.CENTER;setPadding(dp(2),dp(3),dp(2),dp(3));minimumWidth=0;minWidth=0;val raw=text.toString();val br=raw.indexOf('\n');if(br>0&&text !is SpannableString){val s=SpannableString(raw);s.setSpan(RelativeSizeSpan(1.45f),0,br,Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);text=s}}}}
    private fun installTabButtonStyle(){listOf(R.id.tabHome,R.id.tabToday,R.id.tabHistory,R.id.tabAnalytics,R.id.tabSalary,R.id.tabSettings).forEach{id->val tab=rootView.findViewById<TextView>(id)?:return@forEach;tab.setOnTouchListener{_,e->if(e.actionMasked==MotionEvent.ACTION_UP)selectTab(id);false}};syncSelectedTabFromVisiblePanel()}
    private fun syncSelectedTabFromVisiblePanel(){val root=rootView?:return;val content=root.findViewById<LinearLayout>(R.id.contentPanel);val salaryVisible=content?.findViewWithTag<SalaryV2RootView>(SalaryV2RootView.TAG)?.visibility==View.VISIBLE;val id=when{root.findViewById<View>(R.id.celestialHomePanel)?.visibility==View.VISIBLE->R.id.tabHome;root.findViewById<View>(R.id.gpsSettingsPanel)?.visibility==View.VISIBLE->R.id.tabSettings;root.findViewById<View>(R.id.analyticsPdfPanel)?.visibility==View.VISIBLE->R.id.tabAnalytics;salaryVisible->R.id.tabSalary;root.findViewById<View>(R.id.pointageButtons)?.visibility==View.VISIBLE->R.id.tabToday;else->R.id.tabHistory};selectTab(id)}
    private fun selectTab(activeId:Int){listOf(R.id.tabHome,R.id.tabToday,R.id.tabHistory,R.id.tabAnalytics,R.id.tabSalary,R.id.tabSettings).forEach{id->rootView.findViewById<TextView>(id)?.let{val selected=id==activeId;if(it.isSelected!=selected)it.isSelected=selected;styleTab(it,selected)}}}
    ...
    private fun normalizeFrameSizes(){val root=rootView?:return;normalizeRecursive(root);listOf(R.id.tabHome,R.id.tabToday,R.id.tabHistory,R.id.tabAnalytics,R.id.tabSalary,R.id.tabSettings).forEach{id->root.findViewById<TextView>(id)?.let{fitText(it,true)}};ThemeFrameStyler.apply(root)}
