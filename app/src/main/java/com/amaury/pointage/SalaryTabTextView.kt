...
class SalaryTabTextView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : TextView(context, attrs, defStyleAttr) {
    ...
    private fun styleTab(tab:TextView,active:Boolean){
        ...
        if(theme.id=="natural_carbon"){
            // Même cadre métallique que tous les autres contrôles carbone.
            // CarbonCompositeDrawable est désormais cadre seul : aucun fond intérieur.
            tab.backgroundTintList=null
            tab.background=CarbonCompositeDrawable(context)
            tab.elevation=0f
            tab.alpha=1f
            tab.setTextColor(if(active) Color.WHITE else inactive)
            return
        }
        ...
        tab.backgroundTintList=null
        tab.background=LayerDrawable(arrayOf(outer,inner)).apply{setLayerInset(1,dp(5),dp(5),dp(5),dp(5))}
        tab.elevation=0f
        tab.alpha=1f
    ...
    /**
     * Unifie les cadres du thème carbone dans tout l'écran principal.
     * Boutons secondaires, navigation et interrupteurs utilisent la même couronne métallique.
     * Les trois diamants de pointage et la roue Paramètres sont explicitement exclus.
     */
    private fun uniformizeCarbonFrames(){
        if(AppThemeCatalog.current(context).id!="natural_carbon") return
        val root=rootView?:return
        applyCarbonFrameRecursive(root)
        listOf(R.id.tabToday,R.id.tabHistory,R.id.tabAnalytics,R.id.tabSalary,R.id.tabSettings).forEach{id->
            root.findViewById<TextView>(id)?.let { tab ->
                if(tab.background !is CarbonCompositeDrawable){
                    tab.backgroundTintList=null
                    tab.background=CarbonCompositeDrawable(context)
                }
            }
    ...
    private fun applyCarbonFrameRecursive(view:View){
        ...
        val hidden=view.visibility==View.GONE || view.alpha==0f || view.width==1 || view.height==1
        when(view){
            is Button -> if(!protected && !hidden){
                view.backgroundTintList=null
                if(view.background !is CarbonCompositeDrawable) view.background=CarbonCompositeDrawable(context)
            }
            is Switch -> if(!hidden){
                view.backgroundTintList=null
                if(view.background !is CarbonCompositeDrawable) view.background=CarbonCompositeDrawable(context)
        ...
        if(view is ViewGroup) for(i in 0 until view.childCount) applyCarbonFrameRecursive(view.getChildAt(i))
    }
