...
object DeveloperDiamondLivePanel {
    ...
    private fun buttonBg(c: Context) = GradientDrawable().apply {
    ...
    fun show(activity: MainActivity) {
        ...
        val root = LinearLayout(activity).apply {
            ...
            setBackgroundColor(panelColor(activity))
        }
        ...
        root.addView(TextView(activity).apply {
            ...
            setTextColor(if (connected == 3) Color.rgb(50, 170, 80) else Color.rgb(220, 70, 70))
            textSize = 12f
            gravity = Gravity.CENTER
        ...
        fun refreshTargets() {
            targetButtons.forEach { (target, button) ->
                button.alpha = if (target == previewTarget) 1f else .65f
                button.setTextColor(primaryText(activity))
            }
        ...
        val previewButton = Button(activity).apply {
            ...
            setOnClickListener {
                ...
                val message = if (ok) {
                    "Aperçu envoyé sur ${previewTarget.label}"
                } else {
                    "Bouton ${previewTarget.label} introuvable"
                }
        ...
        fun showTab(name: String) {
            ...
            tabButtons.forEach { (tab, button) ->
                button.alpha = if (tab == name) 1f else .68f
                button.setTextColor(primaryText(activity))
            }
        }
        ...
        fun actionButton(label: String) = Button(activity).apply {
            text = label
            isAllCaps = false
            background = buttonBg(activity)
            backgroundTintList = null
        ...
        val reset = actionButton("RÉINITIALISER")
        val close = actionButton("FERMER")
    ...
    private fun addControl(activity: MainActivity, parent: LinearLayout, spec: Spec) {
        ...
        fun setLabel(v: Float) {
            label.text = "${spec.label} : ${formatted(v)}"
        }
    ...
    private fun primaryButtons(activity: MainActivity): List<RedDiamondFinalButton> {
        ...
        for (id in intArrayOf(R.id.entryButton, R.id.pauseButton, R.id.exitButton)) {
            (activity.findViewById<View>(id) as? RedDiamondFinalButton)?.let { result.add(it) }
        }
        return result
    }
    ...
    private fun applyPreview(activity: MainActivity, target: PreviewTarget): Boolean {
        val targets = if (target.id == null) {
            ...
            (activity.findViewById<View>(target.id) as? RedDiamondFinalButton)?.let { listOf(it) } ?: emptyList()
        }
        targets.forEach {
            it.applyLiveDeveloperTuning()
            it.requestLayout()
