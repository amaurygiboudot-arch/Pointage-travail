...
data class StandardButtonLiveConfig(
 ...
 val backgroundImageResName:String="",val frameImageResName:String="")
...
object StandardButtonLiveStyle {
 ...
 fun current(c:Context):StandardButtonLiveConfig { val p=c.getSharedPreferences(PREFS,0); return StandardButtonLiveConfig(
  ...
  p.getString("backgroundImageResName","").orEmpty(),p.getString("frameImageResName","").orEmpty()) }
 ...
 fun setImage(c:Context,frame:Boolean,uri:Uri?):Boolean=c.getSharedPreferences(PREFS,0).edit()
  ...
  .putString(if(frame)"frameImageResName" else "backgroundImageResName","").commit()
 fun setBundledImage(c:Context,frame:Boolean,resName:String?):Boolean=c.getSharedPreferences(PREFS,0).edit()
  ...
  .putString(if(frame)"frameImageUri" else "backgroundImageUri","").commit()
 fun saveCurrent(c:Context):Boolean { val x=current(c); return c.getSharedPreferences(PREFS,0).edit()
  .putInt("backgroundR",x.backgroundR).putInt("backgroundG",x.backgroundG).putInt("backgroundB",x.backgroundB).putInt("backgroundAlpha",x.backgroundAlpha)
  .putInt("frameR",x.frameR).putInt("frameG",x.frameG).putInt("frameB",x.frameB).putInt("frameAlpha",x.frameAlpha).putFloat("frameWidthDp",x.frameWidthDp).putFloat("cornerRadiusDp",x.cornerRadiusDp)
  .putInt("textR",x.textR).putInt("textG",x.textG).putInt("textB",x.textB).putInt("textAlpha",x.textAlpha).putFloat("textSizeSp",x.textSizeSp).putFloat("horizontalPaddingDp",x.horizontalPaddingDp).putFloat("verticalPaddingDp",x.verticalPaddingDp)
  .putInt("backgroundImageAlpha",x.backgroundImageAlpha).putInt("frameImageAlpha",x.frameImageAlpha)
  .putString("backgroundImageUri",x.backgroundImageUri).putString("frameImageUri",x.frameImageUri)
  .putString("backgroundImageResName",x.backgroundImageResName).putString("frameImageResName",x.frameImageResName)
  .putLong("lastExplicitSaveAt",System.currentTimeMillis()).commit() }
 ...
 private fun drawableByName(c:Context,name:String):Drawable? { if(name.isBlank())return null; val id=c.resources.getIdentifier(name,"drawable",c.packageName); return if(id!=0)AppCompatResources.getDrawable(c,id)?.mutate() else null }
 private fun drawableByUri(c:Context,value:String):Drawable? { if(value.isBlank())return null; return runCatching{c.contentResolver.openInputStream(Uri.parse(value))?.use{Drawable.createFromStream(it,"button-live-image")}}.getOrNull()?.mutate() }
 fun applyToButton(c:Context,b:Button){ if(isProtected(b))return; val x=current(c); val d=c.resources.displayMetrics.density
  val base=GradientDrawable().apply{cornerRadius=x.cornerRadiusDp*d;setColor(Color.argb(x.backgroundAlpha,x.backgroundR,x.backgroundG,x.backgroundB));setStroke((x.frameWidthDp*d).roundToInt(),Color.argb(x.frameAlpha,x.frameR,x.frameG,x.frameB))}
  val layers=mutableListOf<Drawable>(base)
  (drawableByName(c,x.backgroundImageResName)?:drawableByUri(c,x.backgroundImageUri))?.also{it.alpha=x.backgroundImageAlpha.coerceIn(0,255);layers.add(it)}
  (drawableByName(c,x.frameImageResName)?:drawableByUri(c,x.frameImageUri))?.also{it.alpha=x.frameImageAlpha.coerceIn(0,255);layers.add(it)}
  b.background=LayerDrawable(layers.toTypedArray()).apply{for(i in 0 until numberOfLayers)setLayerGravity(i,Gravity.FILL)};b.backgroundTintList=null
  b.setTextColor(Color.argb(x.textAlpha,x.textR,x.textG,x.textB));b.textSize=x.textSizeSp;b.isAllCaps=false;b.setPadding((x.horizontalPaddingDp*d).roundToInt(),(x.verticalPaddingDp*d).roundToInt(),(x.horizontalPaddingDp*d).roundToInt(),(x.verticalPaddingDp*d).roundToInt()) }
 ...
 fun bundledDrawableNames():List<String> = R.drawable::class.java.fields.mapNotNull{f->runCatching{f.name}.getOrNull()}.filterNot{it.startsWith("abc_")||it.startsWith("notification_")}.distinct().sorted()
}
...
object DeveloperStandardButtonPanel {
 ...
 fun show(a:MainActivity){
  ...
  root.addView(txt("🎛 RÉGLAGES LIVE — BOUTONS STANDARDS",15f).apply{gravity=Gravity.CENTER});root.addView(txt("Une seule source maître pour cadre, fond, texte et images",11f).apply{setTextColor(secondaryText(a));gravity=Gravity.CENTER})
  ...
  tabs.forEach{tab->
   val tb=devButton(tab);tb.setOnClickListener{showTab(tab)};tabBtns[tab]=tb;row.addView(tb,LinearLayout.LayoutParams(0,dp(40),1f))
   val col=LinearLayout(a).apply{orientation=LinearLayout.VERTICAL}
   specs.filter{it.tab==tab}.forEach{s->addControl(a,col,s)}
   if(tab=="IMAGES"){
    val selected=txt("",11f).apply{setTextColor(secondaryText(a));setPadding(0,dp(4),0,dp(8))}
    fun refreshSelected(){val c=StandardButtonLiveStyle.current(a);selected.text="Fond : ${c.backgroundImageResName.ifBlank{StandardButtonLiveStyle.imageName(a,c.backgroundImageUri)}}\nCadre : ${c.frameImageResName.ifBlank{StandardButtonLiveStyle.imageName(a,c.frameImageUri)}}"}
    refreshSelected();col.addView(selected)
    col.addView(txt("BIBLIOTHÈQUE DU DÉPÔT — touche FOND ou CADRE sous une vignette pour l'appliquer immédiatement.",11f).apply{setTextColor(secondaryText(a));setPadding(0,0,0,dp(6))})
    val grid=GridLayout(a).apply{columnCount=3;alignmentMode=GridLayout.ALIGN_BOUNDS;useDefaultMargins=true}
    StandardButtonLiveStyle.bundledDrawableNames().forEach{name->
     val id=a.resources.getIdentifier(name,"drawable",a.packageName);if(id==0)return@forEach
     val card=LinearLayout(a).apply{orientation=LinearLayout.VERTICAL;gravity=Gravity.CENTER;setPadding(dp(3),dp(3),dp(3),dp(5));background=controlBackground(a)}
     val preview=ImageView(a).apply{setImageResource(id);scaleType=ImageView.ScaleType.CENTER_INSIDE;adjustViewBounds=true;contentDescription=name}
     card.addView(preview,LinearLayout.LayoutParams(-1,dp(64)))
     card.addView(txt(name,9f).apply{gravity=Gravity.CENTER;maxLines=2},LinearLayout.LayoutParams(-1,dp(34)))
     val choose=LinearLayout(a).apply{orientation=LinearLayout.HORIZONTAL}
     val asBg=devButton("FOND").apply{textSize=9f;setOnClickListener{StandardButtonLiveStyle.setBundledImage(a,false,name);applyNow(a);refreshSelected();Toast.makeText(a,"$name appliqué au fond",Toast.LENGTH_SHORT).show()}}
     val asFrame=devButton("CADRE").apply{textSize=9f;setOnClickListener{StandardButtonLiveStyle.setBundledImage(a,true,name);applyNow(a);refreshSelected();Toast.makeText(a,"$name appliqué au cadre",Toast.LENGTH_SHORT).show()}}
     choose.addView(asBg,LinearLayout.LayoutParams(0,dp(34),1f));choose.addView(asFrame,LinearLayout.LayoutParams(0,dp(34),1f));card.addView(choose)
    ...
    clearRow.addView(devButton("RETIRER FOND").apply{setOnClickListener{StandardButtonLiveStyle.setBundledImage(a,false,null);StandardButtonLiveStyle.setImage(a,false,null);applyNow(a);refreshSelected()}},LinearLayout.LayoutParams(0,dp(42),1f))
    clearRow.addView(devButton("RETIRER CADRE").apply{setOnClickListener{StandardButtonLiveStyle.setBundledImage(a,true,null);StandardButtonLiveStyle.setImage(a,true,null);applyNow(a);refreshSelected()}},LinearLayout.LayoutParams(0,dp(42),1f))
    col.addView(clearRow)
   }
   val p=ScrollView(a).apply{addView(col);visibility=View.GONE};pages[tab]=p;host.addView(p)
  }
  val actions=LinearLayout(a).apply{orientation=LinearLayout.HORIZONTAL}
  val save=devButton("ENREGISTRER");val report=devButton("RAPPORT");val reset=devButton("RÉINITIALISER");val close=devButton("FERMER")
  actions.addView(save,LinearLayout.LayoutParams(0,dp(44),1f));actions.addView(report,LinearLayout.LayoutParams(0,dp(44),1f));actions.addView(reset,LinearLayout.LayoutParams(0,dp(44),1f));actions.addView(close,LinearLayout.LayoutParams(0,dp(44),1f));root.addView(actions)
  val dialog=AlertDialog.Builder(a).setView(root).create()
  save.setOnClickListener{if(StandardButtonLiveStyle.saveCurrent(a)){applyNow(a);Toast.makeText(a,"Réglages live enregistrés",Toast.LENGTH_SHORT).show()}else Toast.makeText(a,"Échec de l'enregistrement",Toast.LENGTH_LONG).show()}
  report.setOnClickListener{StandardButtonDeveloperReport.share(a)};reset.setOnClickListener{StandardButtonLiveStyle.reset(a);applyNow(a);dialog.dismiss();show(a)};close.setOnClickListener{dialog.dismiss()}
  dialog.setOnShowListener{dialog.window?.apply{setBackgroundDrawable(android.graphics.drawable.ColorDrawable(Color.TRANSPARENT));clearFlags(WindowManager.LayoutParams.FLAG_DIM_BEHIND);setGravity(Gravity.TOP);setLayout(-1,(a.resources.displayMetrics.heightPixels*.56f).roundToInt())};showTab("FOND");applyNow(a)};dialog.show()
 }
