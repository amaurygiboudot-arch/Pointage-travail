...
object PointageStore {
    ...
    fun entry(context:Context,zoneId:String?=null,zoneAddress:String?=null):Boolean{
        ...
        // Entrée est aussi l'action universelle de reprise : si une journée est déjà
        // ouverte et qu'une pause (manuelle ou automatique) est en cours, on la ferme.
        ...
        val detected=if(zoneId.isNullOrBlank()&&zoneAddress.isNullOrBlank())currentActiveZone(context) else null;val finalId=zoneId?:detected?.first;val raw=zoneAddress?:detected?.second;val finalAddress=raw?.trim()?.takeIf{it.isNotBlank()}?.let{PlaceNames.display(context,it)};val shift=ShiftProfileManager.resolve(context,now);val slot=resolveCompanySlot(context,raw);val companyPause=CompanyBasePauseSettings.baseMinutes(context,slot);val fallback=ShiftProfileManager.pauseMinutes(context,shift);val base=if(companyPause>0)companyPause else fallback
        val changed=synchronized(storageLock){val d=loadUnlocked(context);if(findOpenSession(d)!=null)false else{val item=JSONObject().put("entry",now).put("exit",JSONObject.NULL).put("pauses",JSONArray()).put("shiftType",shift.id).put("companySlot",slot).put("autoPauseMinutes",base);if(!finalId.isNullOrBlank())item.put("zoneId",finalId);if(!finalAddress.isNullOrBlank())item.put("zoneAddress",finalAddress);d.put(item);saveUnlocked(context,d);true}}
        if(!changed)return false;PauseScheduleManager.applyCurrentWindow(context);updateWidgets(context);scheduleIconSync(context);return true
    }
    ...
    /** Ferme toute pause ouverte, quelle que soit son origine. Utilisé par la fin d'un créneau programmé. */
