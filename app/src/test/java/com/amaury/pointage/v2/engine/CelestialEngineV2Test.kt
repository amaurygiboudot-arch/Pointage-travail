...
class CelestialEngineV2Test {
    ...
    fun `nouvelle lune officielle du 17 fevrier 2026 est quasiment noire`() {
        ...
        assertEquals(LunarEclipseStageV2.NONE, snapshot.lunarEclipse.stage)
    ...
    @Test(expected = IllegalArgumentException::class)
    fun `altitude observateur non finie est refusee`() {
        engine.snapshot(
            latitudeDeg = 46.67,
            longitudeDeg = -1.43,
            timeMs = Instant.parse("2026-03-03T11:38:00Z").toEpochMilli(),
            observerAltitudeMeters = Double.NaN
        )
    }

    @Test
    fun `distance lunaire est topocentrique et varie avec observateur`() {
        val at = Instant.parse("2026-02-17T12:01:00Z").toEpochMilli()
        val london = engine.snapshot(51.509, -0.029, at)
        val antipode = engine.snapshot(-51.509, 179.971, at)

        assertTrue(abs(london.moon.distanceKm - antipode.moon.distanceKm) > 5_000.0)
        assertEquals(
            london.moon.altitudeDeg > antipode.moon.altitudeDeg,
            london.moon.distanceKm < antipode.moon.distanceKm
        )
    }
