...
class PaidWorkAllocationV2Test {
    private fun ms(year:Int,month:Int,day:Int,hour:Int):Long = Calendar.getInstance(Locale.FRANCE).apply {
        set(year,month,day,hour,0,0);set(Calendar.MILLISECOND,0)
    }.timeInMillis
    ...
    @Test
    fun unpaidPauseIsRemovedFromTheWeekWhereItOccurs() {
        val previous=TimeZone.getDefault()
        try {
            TimeZone.setDefault(TimeZone.getTimeZone("Europe/Paris"))
            val start=ms(2026,Calendar.SEPTEMBER,6,22)
